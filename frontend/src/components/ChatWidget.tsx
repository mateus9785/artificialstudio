...
interface SendMessageResponse extends ChatMessageRow {
  aiPending: boolean
}
...
function Message({ msg }: { msg: ChatMessageRow }) {
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((data) => {
        const start = data as StartChatResponse
        seenIdsRef.current = new Set(start.messages.map((msg) => msg.id))
        setMessages(start.messages)
        lastIdRef.current = start.messages.at(-1)?.id || 0
        setAiPending(Boolean(start.aiPending))
        setLoaded(true)
      })
      .catch(() => {})
  }, [isOpen, loaded])
    ...
    const interval = setInterval(
      () => {
        api
          ...
          .then((data) => {
            const poll = data as PollMessagesResponse
            mergeMessages(poll.messages)
            setAiPending(Boolean(poll.aiPending))
          })
          .catch(() => {})
      },
  ...
  const sendMessage = useCallback(() => {
    ...
    api
      ...
      .then((data) => {
        const msg = data as SendMessageResponse
        mergeMessages([msg])
