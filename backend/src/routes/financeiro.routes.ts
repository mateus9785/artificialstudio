...
function serializeLancamento(row: LancamentoRow) {
  return {
    id: row.id,
    type: row.type,
...
function validateBody(body: LancamentoBody | undefined): string | null {
...
financeiroRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json((rows as LancamentoRow[]).map(serializeLancamento))
  }),
)
...
financeiroRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { type, description, amount, occurredOn } = req.body as LancamentoBody
    ...
    const [rows] = await pool.query('SELECT * FROM financeiro_lancamentos WHERE id = ?', [(result as { insertId: number }).insertId])
    res.status(201).json(serializeLancamento((rows as LancamentoRow[])[0]))
  }),
)
...
financeiroRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(serializeLancamento((rows as LancamentoRow[])[0]))
  }),
)
