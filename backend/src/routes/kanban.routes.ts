...
interface CardRow {
  ...
  plan_status: 'none' | 'requested' | 'planning' | 'error'
  plan_error: string | null
  run_immediately: number | boolean
  tmux_session: string | null
  error: string | null
  git_watch: number | boolean
...
kanbanRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json((rows as LabelRow[]).map(serializeLabel))
  }),
)
...
kanbanRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await pool.query('SELECT * FROM kanban_labels WHERE id = ?', [(result as { insertId: number }).insertId])
    res.status(201).json(serializeLabel((rows as LabelRow[])[0]))
  }),
)

kanbanRouter.patch(
  '/admin/kanban/labels/:id',
  requireAdmin,
  asyncHandler(async (req, res) => {
    ...
    res.json(serializeLabel((rows as LabelRow[])[0]))
  }),
)
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (code === 'ER_ROW_IS_REFERENCED_2' || code === 'ER_ROW_IS_REFERENCED') {
        const [[{ count }]] = (await pool.query('SELECT COUNT(*) AS count FROM kanban_cards WHERE label_id = ?', [
          req.params.id,
...
kanbanRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json((rows as CardRow[]).map(serializeCard))
  }),
)
...
kanbanRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if ((labelRows as unknown[]).length === 0) {
    ...
    const [rows] = await pool.query(`${CARD_SELECT} WHERE c.id = ?`, [(result as { insertId: number }).insertId])
    res.status(201).json(serializeCard((rows as CardRow[])[0]))
  }),
)

async function loadEditableCard(req: Request, res: Response): Promise<CardRow | null> {
  const [rows] = await pool.query('SELECT * FROM kanban_cards WHERE id = ?', [req.params.id])
  const card = (rows as CardRow[])[0]
  if (!card) {
...
async function loadDeletableCard(req: Request, res: Response): Promise<CardRow | null> {
  const [rows] = await pool.query('SELECT * FROM kanban_cards WHERE id = ?', [req.params.id])
  const card = (rows as CardRow[])[0]
  if (!card) {
    res.status(404).json({ error: 'Card não encontrado.' })
...
kanbanRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(serializeCard((rows as CardRow[])[0]))
  }),
)
...
kanbanRouter.post(
  '/admin/kanban/cards/:id/request-plan',
  requireAdmin,
  asyncHandler(async (req, res) => {
    const [result] = await pool.query(
...
kanbanRouter.post(
  '/admin/kanban/cards/:id/complete',
  requireAdmin,
  asyncHandler(async (req, res) => {
    const [result] = await pool.query(
...
kanbanRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if ((doingRows as unknown[]).length >= MAX_CONCURRENT_CARDS) {
        await conn.rollback()
      ...
      res.json({ card: serializeCard((rows as CardRow[])[0]) })
    } catch (err) {
      await conn.rollback()
      throw err
    } finally {
...
kanbanRouter.post(
  '/admin/kanban/claim-next-to-plan',
  requireAdmin,
  asyncHandler(async (req, res) => {
    const conn = await pool.getConnection()
    try {
...
kanbanRouter.post(
  '/admin/kanban/cards/:id/plan-result',
  requireAdmin,
  asyncHandler(async (req, res) => {
...
kanbanRouter.patch(
  '/admin/kanban/cards/:id/git-watch',
  requireAdmin,
  asyncHandler(async (req, res) => {
...
kanbanRouter.post(
  '/admin/kanban/cards/:id/mark-done',
  requireAdmin,
  asyncHandler(async (req, res) => {
...
kanbanRouter.post(
  '/admin/kanban/cards/:id/mark-error',
  requireAdmin,
  asyncHandler(async (req, res) => {
    const { error } = req.body || {}
    const [result] = await pool.query("UPDATE kanban_cards SET status = 'error', error = ? WHERE id = ? AND status = 'doing'", [
      error || null,
      req.params.id,
    ])
    if ((result as { affectedRows: number }).affectedRows === 0) {
...
interface UsageRow {
  session_used_percent: string | null
  session_resets_at: string | null
  week_used_percent: string | null
...
kanbanRouter.get(
  '/admin/kanban/claude-usage',
  requireAdmin,
  asyncHandler(async (req, res) => {
    const [rows] = await pool.query('SELECT * FROM claude_usage WHERE id = 1')
    res.json(serializeUsage((rows as UsageRow[])[0]))
  }),
)
